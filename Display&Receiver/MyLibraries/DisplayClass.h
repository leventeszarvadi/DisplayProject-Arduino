...
//2017.06.12
class Display{//osztaly a vetitesre
  private:
    int bigNumber;
...
//signal for Segments
    ...
    void displayANumber(int adress, int number);//kiir egy megadott szamot egy megadott cimre
  
  public :

    Display(int * segmentsArray, int * addressArray, bool hyphenSegmentHidden);//constructor
    void setBigNumber(int number)//metodusok amelyekkel inicializalhato az ertekek
    {
      bigNumber=number;  
    }
    ...
    void displayNumbers()//feldolgozza a bealitottt erteket es kiirja a kijelzore
    {
    	if (!clear)
    	{
	    	if (bigNumber>0)
	    	{
	   		...
	   		{
	   			if (!hyphenSegmentHidden)//kotojel
...
Display::Display(int * segmentsArray, int * addressArray, bool hyphenSegmentHiddenValue )//constructor
{
	delayTime=100;
	clear = false;
	bigNumber=0;
...
void Display::displayANumber(int adress, int number)//kiir egy szamot egy cimre
{
